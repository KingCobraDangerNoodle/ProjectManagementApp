...
const listsSlice = createSlice({
  name: 'lists',
  initialState: { user: { username: 'Ozcah', id: '007' }, lists: [] },
  reducers: {
    user(state) {
      state.user.username;
    },
    ...
    getUser(state, action) {
      state.user = action.payload; // payload = return of the async function
    },
  ...
  getUserThunk() {
    return (dispatch) => {
      axios.post('/login').then((response) => {
        dispatch(getUser(response.data)); // response.data == response.body
      });
    };
  },
  createListThunk() {
